...
def my_strategy(day_index, last_profit, last_price):
    ...
    """
    # Model parameters from the framework below (kept locally to avoid edits below)
    A = 120.0
    B = 35.0
    COST = 0.15
    overprod = 1.05  # made = int(1.05 * D)

    # Closed-form optimal price for quadratic objective
    p_star = (A + B * (overprod * COST)) / (2 * B)

    # Clamp to allowed range and return
    return clamp(p_star)
