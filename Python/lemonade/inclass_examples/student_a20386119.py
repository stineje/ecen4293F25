# student_A20386119.py  <-- Rename: replace A12345678 with your own ID
...
def my_strategy(day_index, last_profit, last_price):
    ...
    """
    if day_index != 0:
        if (last_profit / last_price) < 40 and last_price > (MIN_P + 0.10):
            price = last_price - 0.10
        elif (last_profit / last_price) > 100 and last_price < (MAX_P - 0.10):
            price = last_price + 0.10
        else:
            price = last_price
    else:
