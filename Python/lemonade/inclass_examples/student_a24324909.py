# student_A11772053.py  <-- Rename: replace A12345678 with your own ID
...
def my_strategy(day_index, last_profit, last_price):
    ...
    rainy_day_affect = - 0.15 if RAIN[day_index] == 1 else 0.0 # rain reduces price
    temp_affect = 0.005 * (TEMPS[day_index] - 80)  # Higher prices for higher temps
    news_affect = 0.1 * NEWS[day_index]  # Good news increases price, bad news decreases

    price = base_price + rainy_day_affect + temp_affect + news_affect
